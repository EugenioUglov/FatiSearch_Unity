...
public static class EventAggregator
{
    ...
    private static Dictionary<Type, dynamic> SingleInjections = new Dictionary<Type, dynamic>();
    ...
    public static void Inject<T>(Action<T> obj)
    {
        ...
        if (SingleInjections.ContainsKey(key))
        {
            ...
            obj.Invoke(SingleInjections[key]);
        }
        else
        {
            if (DelayedInjection.ContainsKey(key))
...
{
    public abstract void InjectFunc(dynamic call);
}
...
public class InjectAction<T> : ActionContainer
{
    ...
    public override void InjectFunc(dynamic call)
    {
        Inject?.Invoke(call);
    }
}
